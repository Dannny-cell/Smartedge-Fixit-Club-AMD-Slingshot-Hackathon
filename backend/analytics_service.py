...
def get_meeting_sentiment_stats() -> Dict[str, int]:
    """
    Retrieves and aggregates sentiment categories from meeting notes.
    """
    query = "SELECT sentiment FROM meeting_notes"
    conn = get_connection()
    ...
    try:
        ...
        cursor.execute(query)
        for row in cursor.fetchall():
            s_text = (row[0] or "").strip().lower()
            if "positive" in s_text:
                stats["Positive"] += 1
            elif "negative" in s_text:
                stats["Negative"] += 1
            elif "neutral" in s_text:
                stats["Neutral"] += 1
    finally:
        conn.close()
